...
function putStream (cache, opts) {
  ...
  const ret = to((c, n, cb) => {
    ...
    inputStream.end(() => {
      ...
      allDone.then(about => {
        about.digest && ret.emit('digest', about.digest)
        about.size && ret.emit('size', about.size)
        cb()
      }, e => {
        ret.emit('error', e)
      })
  ...
  return (
    ...
    if (exists) {
      ...
      return fixOwner.mkdirfix(
        ...
        return Promise.using(tmpWritten, about => {
          errCheck()
          return moveToDestination(
            tmpTarget, cache, about.digest, opts, errCheck
          ).then(() => about)
        })
      })
    }
